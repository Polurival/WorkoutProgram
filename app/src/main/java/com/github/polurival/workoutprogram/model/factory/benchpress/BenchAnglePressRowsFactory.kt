 ...
 */
class BenchAnglePressRowsFactory(weight: String) : BaseRowsFactory(weight) {

    override fun createTitleRow(resourceManager: ResourceManager): ScheduleRow {
        return ScheduleRow(RowType.HEADER, AdapterModel(resourceManager.getString(R.string.bench_angle_press)))
    }

    override fun calculate(): ArrayList<WorkoutModel> {
        val workoutSchedule = arrayListOf<WorkoutModel>()

        var weight = weight.toDouble() - 27.5
        var repeatCount = 10

        for (weekNumber in 1..12) {
            when (weekNumber) {
                3, 4, 5, 6, 7, 8, 9, 10, 11 -> weight += 2.5
                12 -> weight += 5
            }
            when (weekNumber) {
                3, 8 -> repeatCount -= 2
